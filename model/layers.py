...
class MoConoid(nn.Module):
    ...
    def l1_distance(self, x1, x2):
        distance = F.l1_loss(x1, x2)
        return distance

    # def l1_contrastive_loss(self, pos_1, pos_2, neg_value_list):
    #     criterion = nn.TripletMarginWithDistanceLoss(distance_function=self.l1_distance, margin=1.0)
    #     loss = torch.tensor(0.0, device=self.device)
    #     for neg_value in neg_value_list:
    #         loss += criterion(pos_1, pos_2, neg_value)
    #     return loss
    def l1_contrastive_loss(self, pos_1, pos_2, neg_value_list):

        l_list = list()
        l_pos = torch.sum(F.l1_loss(pos_1, pos_2, reduction='none'), dim=1, keepdim=True)
        l_list.append(torch.reciprocal(l_pos))
        for neg in neg_value_list:
            l_neg = torch.sum(F.l1_loss(pos_1, neg, reduction='none'), dim=1, keepdim=True)
            l_list.append(torch.reciprocal(l_neg))
        logits = torch.cat(l_list, dim=1)
        return self.criterion(logits / self.args.t)
...
class Trainer(object):
    ...
    def evaluate(self, step):
        ...
        target = np.array(  # TODO: I found some of the id missing in the ids_2, pls check why loader will miss them.
        ...
        for epoch in range(start, self.args.epoch):
            for batch_id, (language_id, token_data, id_data) in tqdm(enumerate(all_data_batches[epoch])):
                if language_id == 1:
                    ...
                    if self.args.semantic:
                        semantic_neg_queue1 = self.id_embedding_dict_1[id_data[0].item()].unsqueeze(0)

                        for s, id in enumerate(id_data):
                            if s == 0:
                                continue
                            semantic_neg_queue1 = torch.cat(
                                (semantic_neg_queue1, self.id_embedding_dict_1[id.item()].unsqueeze(0)), 0)

                        self.semantic_neg_queue1.append(semantic_neg_queue1)
                    
                    if is_first1 and len(self.neg_queue1) == self.args.queue_length:
                        is_first1 = False
                        continue
                    ...
                    if len(self.neg_queue1) == self.args.queue_length + 1:
                        ...
                        if self.args.semantic:
                            self.semantic_neg_queue1 = self.semantic_neg_queue1[1:]
                            semantic_neg_queue = self.semantic_neg_queue1
                    ...
                    if self.args.semantic:
                        semantic_neg_queue2 = self.id_embedding_dict_2[id_data[0].item()].unsqueeze(0)

                        for s, id in enumerate(id_data):
                            if s == 0:
                                continue
                            semantic_neg_queue2 = torch.cat(
                                (semantic_neg_queue2, self.id_embedding_dict_2[id.item()].unsqueeze(0)), 0)

                        self.semantic_neg_queue2.append(semantic_neg_queue2)

                    if is_first2 and len(self.neg_queue2) == self.args.queue_length:
                        is_first2 = False
                        continue
                    ...
                    if len(self.neg_queue2) == self.args.queue_length + 1:
                        ...
                        if self.args.semantic:
                            self.semantic_neg_queue2 = self.semantic_neg_queue2[1:]
                            semantic_neg_queue = self.semantic_neg_queue2
                ...
                if self.args.semantic:
                    pos_1 = self.model(neg_queue[0], semantic_neg_queue[0])
                else:
                    pos_1 = self.model(token_data)
                neg_value_list = []

                with torch.no_grad():
                    self._model.eval()
                    if self.args.semantic:
                        pos_2 = self._model(neg_queue[0], semantic_neg_queue[0])
                    else:
                        pos_2 = self._model(token_data)

                    # the first batch is not negative samples
                    for i, neg in enumerate(neg_queue[:-1]):
                        if self.args.semantic:
                            neg_value_list.append(self._model(neg, semantic_neg_queue[i]))
                        else:
                            neg_value_list.append(self._model(neg))

                neg_value = torch.cat(neg_value_list, 0)
                # neg_value = torch.stack(neg_value_list, 0)
                # with mlp head
                if self.args.mlp_head:
                    pos_1 = self.mlp_head(pos_1)
                    pos_2 = self.mlp_head(pos_2)
                    neg_value = self.mlp_head(neg_value)
                # contrastive loss
                # contrastive_loss = self.model.contrastive_loss(pos_1, pos_2, neg_value)
                contrastive_loss = self.model.l1_contrastive_loss(pos_1, pos_2, neg_value_list)
            ...
            self.evaluate(epoch)
        if not os.path.exists(join(PROJ_DIR, 'saved_embeddings')):
            os.makedirs(join(PROJ_DIR, 'saved_embeddings'))
