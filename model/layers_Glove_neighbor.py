...
from loader.DBP15KRawGloveNeighbors import DBP15KRawGloveNeighbors
...
def parse_options(parser):
    ...
    parser.add_argument('--center_norm', type=str2bool, nargs='?',
                        const=True, default=False)
    parser.add_argument('--neighbor_norm', type=str2bool, nargs='?',
                        const=True, default=True)
    parser.add_argument('--emb_norm', type=str2bool, nargs='?',
                        const=True, default=True)
    parser.add_argument('--combine', type=str2bool, nargs='?',
                        const=True, default=True)

    parser.add_argument('--gat_num', type=int, default=1) # 过gat的层数
...
class MyEmbedder(nn.Module):
    def __init__(self, args, vocab_size, padding=ord(' ')):
        ...
        self.attn_mlp = nn.Sequential(
            nn.Linear(Glove_DIM * 2, Glove_DIM),
        )
    ...
    def contrastive_loss(self, pos_1, pos_2, neg_value):
        ...
        # logging.info("l_pos mean: ", torch.mean(l_pos, dim=0).cpu().data)
        l_neg = torch.mm(pos_1.view(bsz, -1), neg_value.t())
        # logging.info("l_neg mean: ", torch.mean(l_neg, dim=0).cpu().data)
...
class BatchMultiHeadGraphAttention(nn.Module):
    def __init__(self, device, args, n_head=MULTI_HEAD_DIM, f_in=Glove_DIM, f_out=Glove_DIM, bias=True):
...
class Trainer(object):
    def __init__(self, training=True, seed=37):
        ...
        loader1 = DBP15KRawGloveNeighbors(self.args.language, "1")
        ...
        loader2 = DBP15KRawGloveNeighbors(self.args.language, "2")
    ...
    def evaluate(self, step):
        ...
        # logging.info("Evaluate at epoch {}...".format(step), file=result_file)
    ...
    def train(self, start=0):
        fix_seed(self.seed)
        print(self.args)
        logging.basicConfig(filename=join(PROJ_DIR, 'log', 'layers_Glove_neighbor/{}_{}_{}_{}_{}_{}_{}_{}_{}_{}.log'.format(
