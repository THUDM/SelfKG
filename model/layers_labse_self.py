...
def parse_options(parser):
    ...
    # LaBSE hyperparamter
    parser.add_argument('--max_length', type=int, default=88)
    parser.add_argument('--negative_sample_num', type=int, default=18000)
    parser.add_argument('--links_ratio', type=float, default=0.01)
...
class Trainer(object):
    ...
    def evaluate(self, step, batch_id, update_temp_links=False):
        print("Evaluate at epoch {} batch_id {}...".format(step, batch_id))
        ...
        if update_temp_links:
            ...
            cand_len = int(D.shape[0]* self.links_ratio)
            candidates = np.argsort(D[:, 0])[: cand_len].tolist()
            change = dict((k, v) for k, v in zip(candidates, I[:, 0][candidates]))

            # key is from dataset_1， v is from dataset_2
            self.temp_links.update(change)

            eval_links = self.temp_links
